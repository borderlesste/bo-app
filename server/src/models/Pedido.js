...
class Pedido {
    ...
    static async updateStatus(id, estado, changedBy, comentario = null) {
        const currentPedido = await this.findById(id);
        if (!currentPedido) return false;
        ...
        await pool.execute(`
            ...
            VALUES (?, ?, ?, ?, ?)
        `, [id, currentPedido.estado, estado, comentario, changedBy]);
    ...
    static async getpedidosWithStatus() {
        const [result] = await pool.execute('SELECT * FROM v_estado_pedidos');
        return result;
    }

    static async generatePedidoNumber() {
        ...
        if (lastPedido.length > 0) {
            const lastNumber = parseInt(lastPedido[0].numero_pedido.split('-')[2]);
