...
const paymentService = {
  ...
  async createPayment(usuario_id, pedido_id, concepto, monto, metodo, estado, referencia, banco_origen) {
    ...
    try {
      ...
      if (estadoCorregido === 'aplicado' || estadoCorregido === 'completado' || estadoCorregido === 'pagado') {
        await this.updateOrderStatusOnPayment(connection, pedido_id, usuario_id, monto);
      }
  ...
  async updatePayment(id, usuario_id, pedido_id, concepto, monto, metodo, estado, referencia, banco_origen) {
    ...
    try {
      ...
      const paymentPedidoId = pedido_id || previousPayment[0].pedido_id;
      ...
      if (wasNotCompleted && isNowCompleted && paymentPedidoId) {
        await this.updateOrderStatusOnPayment(connection, paymentPedidoId, paymentUsuarioId, monto);
      }
  ...
  async updateOrderStatusOnPayment(connection, pedido_id, usuario_id, monto) {
    if (!pedido_id) return;
    ...
    try {
      ...
      if (orderInfo.length === 0) {
        console.log('⚠️ Pedido no encontrado para actualizar estado:', pedido_id);
        return;
      }
      ...
      if (order.usuario_id !== usuario_id) {
        console.log('⚠️ El pago no pertenece al usuario del pedido');
        return;
      }
      ...
      if (order.estado !== 'completado' && isFullyPaid && expectedAmount > 0) {
        ...
        console.log(`✅ Pedido #${pedido_id} actualizado a estado "completado" tras pago completo`);
        ...
        try {
          await connection.execute(
            ...
            [usuario_id, 'pago_completado', `Pedido completado automáticamente tras recibir pago de $${monto}`, 'pedido', pedido_id]
          );
        } catch (logError) {
          console.log('⚠️ Error registrando actividad:', logError.message);
        }
      } else if (!isFullyPaid) {
        console.log(`⏳ Pago parcial registrado. Faltan $${(expectedAmount - totalPagado).toFixed(2)} para completar el pedido.`);
      } else if (order.estado === 'completado') {
        console.log('ℹ️ El pedido ya está completado.');
      }
      
    } catch (error) {
      console.error('❌ Error actualizando estado del pedido tras pago:', error.message);
      throw error;
    }
  },
